...
float4 GetShadowCoord_Custom (float3 positionWS, float4 positionCS) {
...
//#endif
	...
	float3 normalWS                 : TEXCOORD2; // why use a float3 here and half3 in all other passes? why not also in DepthNormalPass?
...
#if defined(REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR)
	output.shadowCoord = GetShadowCoord_Custom(output.positionWS, output.positionCS);
#endif
